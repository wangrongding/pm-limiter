...
const specifiedPM = process.env.npm_config_user_agent ? process.env.npm_config_user_agent.split('/')[0] : null
...
if (process.argv.length < 3) {
  console.log(
    boxen(`${chalk.bold.red('请在scripts中设置你要限制的包管理器: ')}"preinstall":"npx pm-limiter [npm|yarn|...]"`, {
      title: 'Error!',
...
if (specifiedPM != process.argv[2]) {
  console.log(
    boxen(
      `您正在使用 ${chalk.bold.red(PMMap[specifiedPM])} 安装依赖!\n在该项目中,您只能用 ${chalk.bold.green(
        process.argv[2]
      )} 的命令来安装依赖!`,
      {
        title: 'Error!',
